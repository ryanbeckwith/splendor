...
/* Return the number of elements in the vector (which is not necessariy equal
 * to the vector's capacity). */
int Vector::size() {
  return vSize;
}
 ...
 *
 * ERRORS: Calling this function with an index outside the vector's range
 ...
 * causes the program to terminate with an error. We allow an insertion at
 * index vSize because that's equivalent to a call to push_back().
 */
...
// Replaces the element at indexToRemove with the element at indexOfReplacement
void Vector::removeAndReplace(int indexToRemove, int indexOfReplacement) {
  // Change the current index
  vBuffer[indexToRemove] = vBuffer[indexOfReplacement];
  // Erase the value at indexOfReplacement
  erase(indexOfReplacement);
}
